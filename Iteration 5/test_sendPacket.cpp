...
TEST_CASE ("Sending Packet from Floor to Scheduler") {
    ...
    std::vector<uint8_t> packetToSend = floorReader.createData("12:00:00:000", "Up", 1, 5, 4, "None");
...
TEST_CASE ("Sending Packet from Scheduler to Elevator") {
    ...
    std::vector<uint8_t> packetToSend = scheduler.createData(ElevatorEvent(std::tm(), 1, "Up", 5, 4, "None"));
...
TEST_CASE ("Sending Packet from Elevator to Scheduler") {
    ...
    bool isequal = std::equal(success_msg.begin(), success_msg.end(), receivedPacket.begin());
